...
import { FileText, Terminal, Edit, Save, Check, X } from 'lucide-react';
...
export default function TermsPage() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await axios.put(`${API}/pages/terms`, {
        title: editTitle,
    ...
    <div data-testid="terms-page" className="min-h-screen">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex items-center gap-2 mb-2 text-[#1793D1] font-mono text-sm">
          ...
          <span>cat /etc/archhub/terms.md</span>
        </div>

        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <FileText className="w-6 h-6 text-[#1793D1]" />
            <h1 data-testid="terms-page-title" className="text-2xl sm:text-3xl font-extrabold tracking-tighter">
              {page?.title || t('terms_title')}
            </h1>
          </div>
          {isAdmin && !editing && (
            <button data-testid="terms-edit-btn" onClick={() => setEditing(true)}
        ...
        {editing ? (
          <div data-testid="terms-editor" className="space-y-4">
            <input data-testid="terms-title-input" type="text" value={editTitle} onChange={(e) => setEditTitle(e.target.value)}
            ...
            <textarea data-testid="terms-content-editor" value={editContent} onChange={(e) => setEditContent(e.target.value)}
            ...
            <div className="flex items-center gap-2">
              <button data-testid="terms-save-btn" onClick={handleSave} disabled={saving}
              ...
              <button data-testid="terms-cancel-btn"
